#!/usr/bin/env python3
"""
Keyboard Cartesian impedance control

Based on 10_pure_cartesian_impedance_control.py (ported from the C++ version),
with pygame keyboard input added.

Architecture: two threads
  Control thread (1000Hz) -- FK + analytical Jacobian + Cartesian impedance -> joint torques
  Keyboard thread (250Hz) -- pygame detects key presses and updates the Cartesian target pose x_des

Control law (same as C++ pure_cartesian_impedance_control.cpp):
  e_x = [p_des - p ; orientation_error_axis_angle(R_des, R)]
  x_dot = J(q) * dq
  F   = K_cart * e_x - B_cart * x_dot
  tau = J^T(JJ^T + lambda^2 I)^{-1} F + G(q) + f(dq)

Orientation error uses the axis-angle method (same as C++):
  R_err = R_cur^T @ R_des -> angle*axis -> rotated by R_cur back into the world frame

Keyboard control:
  Position (base frame): W/S=X  A/D=Y  Q/E=Z
...
K_ROT = np.array([20.0, 30.0, 40.0])     # Nm/rad, was [10,20,20], significantly lowered
...
# Joint-space damping (the main damping source, directly suppresses joint velocity without J^T amplification)
# Large joints can take more force -> higher damping; small-torque joints -> lower damping
JOINT_DAMPING = np.array([2, 2, 2, 2, 0.8, 0.6])
...
def main():
    print("""
╔══════════════════════════════════════════════════════════════╗
║  Keyboard Cartesian impedance control (axis-angle, C++ port) ║
╠══════════════════════════════════════════════════════════════╣
...
""")
    ...
    # ── Initialize pygame ────────────────────────────────────────
    pygame.init()
    screen = pygame.display.set_mode((520, 138))
    pygame.display.set_caption("Cartesian Impedance (AxisAngle) — focus here!")
    ...
    try:
        ...
        while running:
            ...
            screen.blit(font.render(
                "W/S/A/D/Q/E  I/K/J/L/U/O  Z/X  Space  R  Esc",
            ...
            screen.blit(font.render(
                f"Gripper  des:{g_des_disp:.3f}  act:{g_snap[1]:.3f}  "
                f"{g_pct:.0f}%  [Z=close  X=open]",
