...
Control law:
  ...
  tau = J^T(JJ^T + lambda^2 I)^{-1} F + G(q) + C(q,dq)*dq + f(dq) - B_joint * dq

External force estimation (based on motor torque feedback):
  tau_ext = tau_measured - (G(q) + C(q,dq)*dq + f(dq))
  F_ext = pinv(J^T) @ tau_ext  ->  first-order low-pass filter
...
Keyboard control (horizontal polar):
  ...
  Space=print target pose  R=return home  M=zero wrench  Esc=exit
"""
import time
import threading
import numpy as np
...
K_POS = np.array([10.0, 30.0, 60.0])     # N/m, was [10,20,30]
K_ROT = np.array([10.0, 20.0, 40.0])     # Nm/rad, was [10,20,20], significantly lowered
K_CART = np.concatenate([K_POS, K_ROT])

# Cartesian space damping (keep small! Large values amplify velocity noise via J^T@B@J)
B_POS = np.array([0.0, 0.0, 0.0])     # N.s/m, only a small amount of Cartesian damping
B_ROT = np.array([0.0, 0.0, 0.0])     # Nm.s/rad, main damping comes from JOINT_DAMPING
...
# Velocity low-pass cutoff frequency (Hz), suppresses dq high-frequency noise amplified by the Jacobian
DQ_LPF_CUTOFF = 40.0

# Whether to enable Coriolis compensation (sensitive to dq noise, try disabling if needed)
...
FT_CUTOFF_FREQ = 5.0   # Hz, lower is smoother but has more delay
...
║    M       ->  Zero the external-force sensor (set current   ║
║                value as the zero point)                       ║
...
""")
    ...
    disp_wrench = np.zeros(6)   # Filtered external wrench [Fx,Fy,Fz,Mx,My,Mz]
    wrench_bias = np.zeros(6)   # Zero offset triggered by the M key
    ...
    def impedance_loop():
        ...
        # dq low-pass filter state
        dq_filtered = np.zeros(6)
        alpha_dq = (2 * np.pi * DQ_LPF_CUTOFF * dt) / (1 + 2 * np.pi * DQ_LPF_CUTOFF * dt)
        ...
        while not stop_event.is_set():
            ...
            if ctrl_enabled.is_set():
                ...
                dq_raw = np.array(robot.get_current_vel())

                # First-order low-pass filter for dq
                if not dq_initialized:
                    dq_filtered[:] = dq_raw
                    dq_initialized = True
                else:
                    dq_filtered[:] = alpha_dq * dq_raw + (1 - alpha_dq) * dq_filtered
                dq = dq_filtered
                ...
                # pinv(J^T) @ tau_ext  (DLS regularization)
                JJT_ft = J @ J.T
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        # Use the current filtered external wrench as the zero offset
                        with disp_lock:
                            wrench_bias[:] = disp_wrench + wrench_bias
                        print("[M] External-force sensor zeroed")
